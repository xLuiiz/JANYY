(function(window){
    ...
    Seed.prototype = {
        ...
        drawText: function() {
            ...
            ctx.fillText("Clique aqui:) ", 30, -5);
            ctx.fillText("Preciso te fazer uma pergunta !", 28, 10);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
    ...
    Tree.prototype = {
        ...
        initSparkle: function() {
            var sparkle = this.opt.sparkle || {};
            var cache = [],
                num = sparkle.num || 650,
                width = sparkle.width || this.width,
                height = sparkle.height || this.height;
            var r = 240, x, y;
            for (var i = 0; i < num; i++) {
                cache.push(this.createSparkle(width, height, r));
            }
            this.sparkles = [];
            this.sparklesCache = cache;
        },

        initBubble: function() {
            var bubble = this.opt.bubble || {};
            var cache = [],
                num = bubble.num || 100,
                width = bubble.width || this.width,
                height = bubble.height || this.height;
            for (var i = 0; i < num; i++) {
                cache.push(this.createBubble(width, height));
            }
            this.bubbles = [];
            this.bubblesCache = cache;
        },
        ...
        removeSparkle: function (sparkle) {
            var sparkles = this.sparkles;
            for (var i = 0; i < sparkles.length; i++) {
                if (sparkles[i] === sparkle) {
                    sparkles.splice(i, 1);
                }
            }
        },
        ...
        createSparkle: function(width, height, radius) {
            var x, y;
            while (true) {
                x = random(20, width - 20);
                y = random(20, height - 20);
                if (inheart(x - width / 2, height - (height - 40) / 2 - y, radius)) {
                    return new Sparkle(this, new Point(x, y));
                }
            }
        },
        ...
        createBubble: function(width, height) {
            ...
            return new Bubble(this, new Point(x, y), vx, vy, radius, alpha);
        },
        ...
        flower: function(num) {
            ...
            var sparkles = s.sparklesCache.splice(0, num);
            for (var k = 0; k < sparkles.length; k++) {
                s.addSparkle(sparkles[k]);
            }
            sparkles = s.sparkles;
            for (var l = 0; l < sparkles.length; l++) {
                sparkles[l].flower();
            }
        },
        ...
        jump: function() {
            var s = this, blooms = s.blooms, sparkles = s.sparkles, bubbles = s.bubbles;
            if (blooms.length) {
                for (var i = 0; i < blooms.length; i++) {
                    blooms[i].jump();
                }
            ...
            if ((bubbles.length && bubbles.length < 10) || !bubbles.length) {
                var bubble = this.opt.bubble || {},
                    width = bubble.width || this.width,
                    height = bubble.height || this.height;
                for (var i = 0; i < random(1,3); i++) {
                    bubbles.push(this.createBubble(width, height));
                }
            }
        },
    ...
    Bloom = function(tree, point, figure, color, alpha, angle, scale, place, speed) {
        this.tree = tree;
        this.point = point;
        this.color = color || (random(0, 2) === 0 ? '#48c6f0' : random(0, 1) === 0 ? 'rgb(255, 255, 255)' : '#ff62be');
    ...
    Sparkle = function(tree, point, color, alpha, scale, place, speed) {
        this.tree = tree;
        this.point = point;
        this.color = color || (random(0, 1) === 0 ? '#FFFFFF' : '#ff9f9f');
    ...
    Bubble = function(tree, point, vx, vy, radius, alpha) {
        this.tree = tree;
        this.point = point;
        this.vx = vx;
        this.vy = vy;
